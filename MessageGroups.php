...
class MessageGroups {
	...
	public function recache() {
		...
		foreach ( $this->getCacheGroupLoaders() as $c ) {
			$c->recache();
		}
	...
	public static function clearCache() {
		...
		foreach ( $self->getCacheGroupLoaders() as $c ) {
			$c->clearCache();
		}
	 ...
	 * and may additionaly implement CachedMessageGroupLoader or DatabaseMessageGroupLoader
	 * @param string|null $name Loader class name
	 * @return MessageGroupLoader[]
	 */
	protected function getGroupLoaders( $name = null ) {
		if ( $this->groupLoaders === null ) {
			$cache = $this->getCache();

			$groupLoaderInstance = $this->groupLoaders = [];
			Hooks::run( 'TranslateInitGroupLoaders', [ &$groupLoaderInstance ] );

			if ( $groupLoaderInstance !== [] ) {
				foreach ( $groupLoaderInstance as $g ) {
					if ( !$g instanceof MessageGroupLoader ) {
					...
					if ( $g instanceof DatabaseMessageGroupLoader ) {
						$g->setDatabase( TranslateUtils::getSafeReadDB() );
					}

					if ( $g instanceof CachedMessageGroupLoader ) {
						$g->setCache( new MessageGroupWANCache( $cache ) );
					}

					$this->groupLoaders[get_class( $g )] = $g;
				}
			}
		}

		if ( $name !== null ) {
			if ( !isset( $this->groupLoaders[ $name ] ) ) {
				throw new \InvalidArgumentException( "Group loader - '$name' not found." );
			}

			return $this->groupLoaders[ $name ];
		}
	...
	public static function labelExists( $name ) {
		$g = self::singleton()->getGroupLoaders( 'AggregateMessageGroupLoader' );
		$groups = $g->loadAggregateGroups();
