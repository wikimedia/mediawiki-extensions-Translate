...
abstract class MessageGroupBase implements MessageGroup {
	...
	private function getArrayInsertables() {
		...
		if ( isset( $insertableConf['class'] ) || isset( $insertableConf['classes'] ) ) {
			throw new \RuntimeException( "Invalid configuration specified for Insertables." );
		}
		foreach ( $insertableConf as $config ) {
			if ( !isset( $config['class'] ) ) {
				throw new \RuntimeException( "Insertable configuration does not provide a class." );
			}
