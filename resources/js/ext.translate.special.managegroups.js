( function () {
	...
	$( function () {
		...
		$( '.smg-rename-rename-action' ).click( function () {
			...
			getRenames( keyData.groupId, keyData.msgKey, keyData.languageCode )
				.done( function ( data ) {
				// Open the dialog, and display possible renames.
					windowManager.openWindow( renameDialog, {
						...
						languageCode: keyData.languageCode
					} );
		...
		$( '.smg-rename-new-action' ).click( function () {
			var keyData = RenameDropdown.getData();
			setAsNew( keyData.groupId, keyData.msgKey, keyData.languageCode ).done( function ( data ) {
	 ...
	 */
	function getRenames( groupId, msgKey, languageCode ) {
		...
		params = {
			...
			formatversion: 2,
			mmggroup: groupId,
			mmglanguage: languageCode,
			mmgkey: msgKey,
			mmggrouptype: getGroupType()
		};
	...
	/**
	 * Identifies and returns the group type from the URL
	 * @return {string}
	 */
	 ...
	 * @param {string} renameParams.languageCode
	...
	function setRename( renameParams ) {
		...
		params = {
			...
			languageCode: renameParams.languageCode,
	 ...
	 * @return {jQuery.Promise}
	 */
	function setAsNew( groupId, msgKey, languageCode ) {
		var params, api = new mw.Api();
