( function ( $, mw ) {
	 ...
	 * @return {Array} namespaces Array of valid namespaces
	 */
	...
	$( document ).ready( function () {
		...
		$( '#action-save' ).click( function () {
			...
			savePage( pageName, pageContent ).done( function () {
				pageUrl = serverName + '/wiki/' + pageName + '?action=edit';
				$( '.messageDiv' ).html( 'The page was saved. You can <a href="' + pageUrl +
					'">edit</a> it.' ).show();
				$( '#action-prepare' ).show();
				$( '#title' ).val( '' );
				$( '#action-save' ).hide();
		...
		$( '#action-prepare' ).click( function () {
			...
			$.when( getPageContent( pageName ) ).done( function ( content ) {
				...
				$.when( getDiff( pageName, pageContent ) ).done( function ( diff ) {
					...
					$( '.divDiff' ).show( 'fast' );
					if ( diff !== '' ) {
						$( '.messageDiv' ).html( 'The page was prepared for translation. ' +
							'See the diff below. Click the \'Save\' button if okay!' ).show();
						$( '#action-prepare' ).hide();
						$( '#action-save' ).show();
					} else {
						$( '.messageDiv' ).html( 'It seems the page has already been prepared ' +
							'for translation. No changes compared to previous version.').show();
					}
				} );
			} );
		} );
