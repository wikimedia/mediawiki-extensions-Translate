( function ( $, mw ) {
	...
	function importHandler() {
		...
		errorBox.hide( 'fast' );
		if ( pageTitle === '' ) {
			errorBox.html( mw.msg( 'pm-pagetitle-missing' ) ).show( 'fast' );
			return;
		}
		if ( slashPos !== -1 ) {
		...
		if ( pageName === '' ) {
			errorBox.html( mw.msg( 'pm-pagename-missing' ) ).show( 'fast' );
			return;
		}
		if ( langCode === '' ) {
			errorBox.html( mw.msg( 'pm-langcode-missing' ) ).show( 'fast' );
			return;
		}
