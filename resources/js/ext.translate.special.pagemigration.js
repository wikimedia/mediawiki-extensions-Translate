( function ( $, mw ) {
	...
	function splitTranslationPage( fuzzyTimestamp, pageTitle ) {
		...
		return api.get( {
			...
			if ( typeof obj === undefined ) {
				...
				$( '.mw-tpm-sp-error-unit__message' ).text( mw.msg( 'pm-page-does-not-exist', pageTitle ) )
					.show( 'fast' );
				return new $.Deferred().reject();
			}
			if ( obj.revisions === undefined ) {
				// the case of /en subpage where first edit is by FuzzyBot
				$( '.mw-tpm-sp-error-unit__message' ).text( mw.msg( 'pm-old-translations-missing', pageTitle ) )
					.show( 'fast' );
				return new $.Deferred().reject();
			}
	...
	$( '#action-save' ).click( function () {
		...
		if ( noOfSourceUnits < noOfTranslationUnits ) {
			$( '.mw-tpm-sp-error-unit__message' ).text( mw.msg( 'pm-extra-units-warning' ) )
				.show( 'fast' );
			return;
		} else {
	...
	$( '.mw-tpm-sp-unit-listing' ).ready( function () {
		...
		$( '#action-import' ).click( function () {
			...
			$( '.mw-tpm-sp-error-unit__message' ).hide( 'fast' );
			if ( pageName === '' ) {
				$( '.mw-tpm-sp-error-unit__message' ).text( mw.msg( 'pm-pagename-missing' ) )
					.show( 'fast' );
				return;
			}
			if ( langCode === '' ) {
				$( '.mw-tpm-sp-error-unit__message' ).text( mw.msg( 'pm-langcode-missing' ) )
					.show( 'fast' );
				return;
			}
