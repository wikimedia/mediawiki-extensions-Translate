...
( function ( $, mw ) {
	...
	$( document ).ready( function () {
		...
		excludeFunction = function( event ) {
			...
			$( event.target ).closest( '.mw-tpa-group' ).find( 'li' ).each( function (key, value) {
				// Need to trim to remove the trailing whitespace
				// innerText isn't supported by Firefox
				exclude.push( value.textContent.trim() );
			} );
		};
		...
		$( '#tpt-aggregategroups-save' ).on( 'click', function () {
			...
			successFunction = function ( data ) {
				...
				$removeSpan = $( '<span>' ).prop( 'id', aggregateGroupId )
					.addClass( 'tp-aggregate-remove-ag-button' );
				$editSpan = $( '<span>' ).prop( 'id', aggregateGroupId )
