		...
		displayEmptyListHelp: function () {
			...
			if ( messageTable.mode === 'proofread' ) {
				if ( selectedTab === 'all' ) {
					...
					$actions.append( messageTable.otherActionButton( 'tux-empty-list-other-action', function () {
						mw.translate.changeFilter( $( '.tux-tab-unproofread' ).click() );
					} ) );
					// @todo View all
					...
					$actions.append( messageTable.otherActionButton( 'tux-empty-list-translated-action', function () {
						messageTable.switchMode( 'translate' );
					} ) );
				} else if ( selectedTab === 'unproofread' ) {
					$emptyListHeader.text( mw.msg( 'tux-empty-nothing-new-to-proofread' ) );
					$guide.text( mw.msg( 'tux-empty-you-can-help-providing' ) );
					$actions.append( messageTable.otherActionButton( 'tux-empty-you-can-review-already-proofread', function () {
						mw.translate.changeFilter( $( '.tux-tab-translated' ).click() );
					} ) );
				}
			} else {
				if ( selectedTab === 'all' ) {
