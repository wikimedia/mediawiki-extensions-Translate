...
class TranslateUtils {
	...
	public static function translationChanges(
		...
		$commonFields = array( "{$t}_timestamp AS timestamp", "{$t}_user_text AS user_text" );
		$options = "ORDER BY lang ASC, timestamp DESC";
		$join_conds = "";
		if ( $t === 'rc' ) {
			$fields = array_merge(
				array( "{$t}_title AS title",
				"{$t}_namespace AS namespace",
				"substring_index(title, '/', -1) as lang" ),
				$commonFields,
				$extraFields
			);
			$options = "HAVING timestamp >= '{$cutoff}' " .
			...
			$join_conds = "JOIN page ON rev_page = page_id" .
				...
				"AND rev_parent_id = 0";
			$options = "HAVING timestamp >= '{$cutoff}' " . $options;
		}
