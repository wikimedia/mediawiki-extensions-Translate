...
class MessageGroupStatesUpdaterJobTest extends MediaWikiTestCase {
	...
	public function testHooks() {
		...
		$status = $page->doEdit( 'trans1', __METHOD__, 0, false, $user );
		self::runJobs();
		$currentState = ApiGroupReview::getState( $group, 'fi' );
		...
		// Second translation
		$page = WikiPage::factory( Title::newFromText( 'MediaWiki:key2/fi' ) );
		$status = $page->doEdit( 'trans2', __METHOD__, 0, false, $user );
		self::runJobs();
		$currentState = ApiGroupReview::getState( $group, 'fi' );
