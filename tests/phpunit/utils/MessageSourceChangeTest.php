...
class MessageSourceChangeTest extends PHPUnit\Framework\TestCase {
	...
	public function testBreakRename() {
		...
		$this->assertNull( $added,
			'renamed message is removed from additions.' );
		$this->assertNull( $deleted,
			'renamed message is removed from deletions.' );
		...
		$this->assertArrayHasKey( 'key', $added,
			'broken rename message is added back to additions' );
		$this->assertArrayHasKey( 'key', $deleted,
			'broken rename message is added back to deletions' );
	}
	...
	public function testPreviousState() {
		...
		$this->assertNull( $changed,
			'broken rename message with previous state as NONE is not found in changes' );
		$this->assertArrayHasKey( 'key', $added,
			'broken rename message with previous state as additions for renamed is found ' .
			'in additions' );
	}
