...
class ApiManageMessageGroups extends ApiBase {
	...
	public function execute() {
		...
		if ( !MessageChangeStorage::isValidCdbName( $name ) ) {
			throw new InvalidArgumentException( "Invalid changeset file name passed - '$name'. " );
		}
	...
	protected function handleRename( MessageGroup $group, MessageSourceChange $sourceChanges,
		...
		foreach ( $languages as $code ) {
			...
			// Check for the renamed message only in the rename list because we
			// want the old content of the message, and not the new modified
			// incoming translation. Also old wiki messages are tracked
			// only for renames. For others we only track incoming changes.
			...
			if ( $renameMsg === null ) {
				...
				// As there was no change with this message before, we set state to none.
			...
			} elseif ( $renameMsgState !== MessageSourceChange::M_NONE ) {
	...
	protected function handleNew( MessageSourceChange $sourceChanges, $msgKey, $sourceLang ) {
		...
		foreach ( $languages as $code ) {
			...
			if ( $code === $sourceLang && $msg === null ) {
				throw new InvalidArgumentException(
					'Message key passed for new addition was not found in the renamed message ' .
					'list for source language.'
				);
			}

			if ( $code === $sourceLang && $msgState !== MessageSourceChange::M_RENAME ) {
				throw new InvalidArgumentException(
