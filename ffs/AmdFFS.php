 ...
 *
 * A limitation is that it only accepts json compatible structures inside the define wrapper function.
 * For example the following example is not ok since there are no quotation marks around the keys:
 * define({ 
 *   key1: "somevalue",
 *   key2: "anothervalue"
 * });
 ...
 * });
 *
 * Note that it does not support exporting with the root construction, there is only support for reading it.
 * However, this is not a serious limitation as Translatewiki doesn't export the base language.
 *
 * @file
