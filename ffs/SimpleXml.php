<?php
/**
 * Support for a simple XML format that is basically just a standard XML file
 * which contains localization in format of:
 * <?xml version="1.0" encoding="UTF-8"?>
 * <resource>
 *   <string name="name">English</string>
 *   <!-- more keys -->
 * </resource>
 * This format is similar to Android format and is almost compatible, but unlike
		...
		foreach ( $reader as $element ) {
			...
			if ( $element->getName() === 'string' ) {
				...
				foreach ( $element as $item ) {
					$forms[(string)$item['quantity']] = $item;
				}
		...
		foreach ( $collection as $key => $m ) {
			...
			if ( strpos( $value, '{{PLURAL' ) === false ) {
				...
				foreach ( $forms as $quantity => $content ) {
					$item = $element->addChild( 'item',  $content );
