		...
		foreach ( $langs as $l ) {
			...
			switch ( $type ) {
				...
				case 'namespace':
					...
					break;
				default:
					$this->output( 'Invalid type: must be one of special, magic, namespace.' );
					exit( 1 );
			}
			...
			if ( strpos( $export, '#!!' ) !== false ) {
				$this->output( "There are warnings with $l." );
			}
			...
			if ( $count ) {
				...
				$this->output( "Adding new entry to $outFile, please double check location.", $l );
				$pos = strpos( $data, "*/" );
				if ( $pos === false ) {
					$this->output( ". FAILED! Totally new file? No header?", $l );
				} else {
					$pos += 3;
				}
