...
class CommandlineExport extends Maintenance {
	public function __construct() {
		...
		$this->addOption(
			'skipgroup',
			'Comma separated list of group IDs that should not be exported',
			false, /*required*/
			true /*has arg*/
		);
		$this->addOption(
		...
		$this->addOption(
			'hours',
			'Only export languages with changes in the last given number of hours',
			false, /*required*/
			true /*has arg*/
		);
		$this->addOption(
			'ppgettext',
			'Group root path for checkout of product. "msgmerge" will post ' .
		...
		$this->addOption(
			'no-location',
			'Only used combined with "ppgettext". This option will rebuild ' .
		...
		$this->addOption(
			'core-meta',
			'Allow export of specific MediaWiki core meta groups (translatewiki.net specific)',
			false, /*required*/
			false /*has arg*/
		);
	}
	...
	public function execute() {
		...
		if ( !is_writable( $target ) ) {
			$this->output( "Target directory is not writable (" . $target . ").\n" );
			exit( 1 );
		}
		...
		if ( !count( $groups ) ) {
			$this->output( "EE1: No valid message groups identified.\n" );
			exit( 1 );
		}
		...
		foreach ( $groups as $groupId => $group ) {
			...
			if ( $group instanceof FileBasedMessageGroup ) {
				...
				foreach ( $langs as $lang ) {
					...
					if ( $definitionFile ) {
						if ( is_file( $definitionFile ) ) {
							...
							$this->output( $definitionFile . " does not exist.\n" );
							exit( 1 );
						}
					}
				}
				...
				if ( $noFuzzy ) {
					$this->output( "--no-fuzzy is not supported for this message group.\n" );
				}
