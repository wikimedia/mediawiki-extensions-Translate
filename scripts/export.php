...
class CommandlineExport extends Maintenance {
	...
	public function execute() {
		...
		if ( !is_writable( $target ) ) {
			$this->output( "Target directory is not writable ($target).", 1 );
		}
		...
		foreach ( $groups as $groupId => $group ) {
			...
			if ( $group->isMeta() ) {
				if ( !$coreMeta ) {
					$this->error( "Skipping meta message group $groupId." );
