...
class TS extends Maintenance {
	public function execute() {
		...
		echo "username\tregistration\tedit count\tis translator?\tpromoted\n";
		...
		foreach ( $rejected as $r ) {
			echo "{$r->log_title}\t{$r->log_timestamp}\t0\t\t\n";
		}
		...
		foreach ( $users as $u ) {
			...
			foreach ( $logs as $log ) {
				if ( $log->log_action === 'promoted' ) {
					...
					} elseif ( isset( $data['5::newgroups'] ) && in_array( 'translator', $data['5::newgroups'] ) ) {
						$promoted = $log->log_timestamp;
						break;
					}
				}
			...
			echo "{$u->user_name}\t{$u->user_registration}\t{$u->user_editcount}\t{$u->ug_group}\t{$promoted}\n";
		}
	}
}
