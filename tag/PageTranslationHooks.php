	...
	public static function languages( $data, $params, $parser ) {
		...
		if ( $isSourcePage ) {
			if ( $canmark && $parser->getUser()->isAllowed( 'pagetranslation' ) ) {
				...
				if ( $marked ) {
					$downButton = Html::openElement( 'li', array( 'class' => 'mw-translate-action' ) );
		...
		$langBar .= HTML::openElement( 'div', array(
			'class' => 'mw-translate-langbar-container mw-translate-langbar-container-'
		...
		foreach ( $output as $code => $percent ) {
			...
			$langCell = Html::rawElement( 'li', array( 'class' => 'mw-translate-language' ),
				"$name&#160;$percentImage"
			);

			if ( $currentTitle->equals( $subpage ) ) {
				$langCell = Html::rawElement( 'li', array(
						'class' => 'mw-translate-language mw-translate-selected'
					),
					"$name&#160;$percentImage"
				);
