...
class PageTranslationHooks {
	...
	public static function onGetUserPermissionsErrorsExpensive( Title $title, User $user,
		...
		$error = self::shouldRestrictTranslations( $handle );
		if ( $error ) {
			$result = $error;
			return false;
		}
	 ...
	 * @return bool|array array containing error message if restricted, false otherwise
	 */
	private static function shouldRestrictTranslations( MessageHandle $handle ) {
