...
class MessageSourceChange {
	 ...
	 * @return void
	 */
	public function addRename( $language, $addedMessage, $deletedMessage, $similarity ) {
	 ...
	 * @return array|null
	 */
	public function findMessage( $language, $key, &$modificationType, $possibleStates = [] ) {
		$allChanges = [];
	 ...
	 */
	public function breakRename( $languageCode, $msg ) {
		...
		// Remove them from the renames arraremoveAdditionsy
	 ...
	 * @return bool
	 */
	public function isPreviousState( $languageCode, $key, $type ) {
		$msgType = null;
		$msg = $this->findMessage( $languageCode, $key, $msgType, [ self::M_RENAME ] );
		if ( isset( $msg['previous_state'] ) && $msg['previous_state'] === $type ) {
			return true;
		}
	...
	public function getSimilarity( $languageCode, $key ) {
		$msg = $this->getMatchedMsg( $languageCode, $key );
		if ( !$msg ) {
			return null;
		}
