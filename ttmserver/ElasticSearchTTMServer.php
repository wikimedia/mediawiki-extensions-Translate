...
GROOVY;
	...
	public function search( $queryString, $opts, $highlight ) {
		...
		try {
			...
			} while ( $resultset->getTotalHits() > count( $terms ) );//Break when all elements are found
	...
	/* Do two things:
	 * 1. Find message in ES index using message keys
	 *    and scores from search(). Feed the messages
	 *    to MessageCollection to make sure these are translated
	 *    messages, if not found, fetch next few messages from index
	 *    until the limit.
	 * 2. Also retrieves facets counts
	 */
...
GROOVY;
		...
		try {
			do {
				...
				// Use Message Collection to make sure the message keys are translated
				foreach ( $results->getResults() as $document ) {
					$data = $document->getData();
					$localid = $data['localid'];
					$localid = explode( ':', $localid );
					$messages[$localid[1]] = $data['content'];
				}

				$language = $opts->getValue( 'language' );
				// Hardcoded namespace. I wonder how can we pass an array of namespaces?
				$definitions = new MessageDefinitions( $messages, NS_MEDIAWIKI );
				...
				&& count( $collect ) < $results->getTotalHits()
