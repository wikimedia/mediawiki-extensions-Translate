...
GROOVY;
		...
		do {
			...
			$offset = $query->getParam( 'from' );
			$query->setFrom( $offset + $limit );

		} while ( count( $collect ) <  $limit
			&& ( count( $collect ) + $offset < $results->getTotalHits() )
		);

		$collect = array_slice( $collect, 0, $limit );
