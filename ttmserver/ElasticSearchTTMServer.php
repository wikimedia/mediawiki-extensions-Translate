...
class ElasticSearchTTMServer extends TTMServer implements ReadableTTMServer, WritableTTMServer {
	...
	public function update( MessageHandle $handle, $targetText ) {
		...
		if ( $targetText !== null ) {
			...
			try {
				...
				error_log( __METHOD__ . ': update failed' );
				return false;
			}
		}
	...
	protected function waitUntilReady() {
		...
		while ( true ) {
			...
			if ( $response->hasError() ) {
				$this->logOutput( 'Error fetching index health but going to retry.  Message: ' + $response->getError() );
				sleep( 1 );
				continue;
			}
			...
			if ( $each === 0 || $active === $expectedActive ) {
				$this->logOutput( "tactive:$active/$expectedActive relocating:$relocating " .
	...
	public function batchInsertTranslations( array $batch ) {
		...
		foreach ( $batch as $key => $data ) {
			...
			$docs[] = $this->createDocument( $handle, $text, $this->revIds[$key] );
		}
