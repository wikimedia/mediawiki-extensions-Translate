...
class SpecialSupportedLanguages extends SpecialPage {
	...
	protected function makeUserList( $users, $editcounts, $lastedits ) {
		...
		foreach ( $users as $username => $count ) {
			...
			if ( isset( $editcounts[$username] ) ) {
				...
				$last = round( $last / $day );
				$attribs['title'] = $this->msg( 'supportedlanguages-activity' )->params( $username )->numParams( $count, $last )->text();
		...
		$html .= $this->msg( 'supportedlanguages-translators' )->params( $linkList,	count( $links ) )->text();
		$html .= "</p>\n";
	...
	protected function getColorLegend() {
		...
		for ( $i = 0; $i <= $period; $i += 30 ) {
			$iFormatted = $this->getLanguage()->formatNum( $i );
