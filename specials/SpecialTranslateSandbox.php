	...
	<div class="row tsb-body">
		<div class="four columns pane requests">
			...
			<div class="request-footer">
				<span class="selected-counter">{$this->msg( 'tsb-selected-count' )->numParams( 0 )->escaped()}</span>
				&nbsp;
...
HTML;
	...
	protected function makeRequestItem( $request ) {
		...
		$nameEncForId = preg_replace( '/ /', '_', $nameEnc );
...
HTML;
	...
	public static function translatorRequestSort( $a, $b ) {
		$aTranslationCount = $a['translations'];
		$bTranslationCount = $b['translations'];

		$translationCountDiff = $bTranslationCount - $aTranslationCount;
		...
		if ( $translationCountDiff === 0 ) {
			...
			return strcmp( $a['registrationdate'], $b['registrationdate'] );
		}
