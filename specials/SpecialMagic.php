...
class SpecialMagic extends SpecialPage {
	...
	function getDescription() {
		return $this->msg( 'translate-magic-pagename' )->plain();
	}
	...
	protected function getForm() {
		...
		$form = Xml::tags( 'form',
			...
			'<table><tr><td>' .
				$this->msg( 'translate-page-language' )->plain() .
			'</td><td>' .
				TranslateUtils::languageSelector( $this->getLanguage()->getCode(), $this->options['language'] ) .
			'</td></tr><tr><td>' .
				$this->msg( 'translate-magic-module' )->plain() .
			'</td><td>' .
				$this->moduleSelector( $this->options['module'] ) .
			'</td></tr><tr><td colspan="2">' .
				Xml::submitButton( $this->msg( 'translate-magic-submit' )->plain() ) . ' ' .
				Xml::submitButton( $this->msg( 'translate-magic-cm-export' )->plain(), array( 'name' => 'export' ) ) .
			'</td></tr></table>' .
	...
	protected function moduleSelector( $selectedId ) {
		...
		foreach ( $this->aModules as $code ) {
			$selector->addOption( $this->msg( 'translate-magic-' . $code )->plain(), $code );
		}
