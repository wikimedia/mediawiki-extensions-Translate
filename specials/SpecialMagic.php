...
class SpecialMagic extends SpecialPage {
	...
	function getDescription() {
		return $this->msg( 'translate-magic-pagename' )->escaped();
	}
	...
	protected function getForm() {
		...
		$form = Xml::tags( 'form',
			...
			'</td></tr><tr><td colspan="2">' .
				Xml::submitButton( $this->msg( 'translate-magic-submit' )->escaped() ) . ' ' .
				Xml::submitButton( $this->msg( 'translate-magic-cm-export' )->escaped(), array( 'name' => 'export' ) ) .
			'</td></tr></table>' .
	...
	protected function moduleSelector( $selectedId ) {
		...
		foreach ( $this->aModules as $code ) {
			$selector->addOption( $this->msg( 'translate-magic-' . $code )->escaped(), $code );
		}
	...
	protected function setup( $parameters ) {
		...
		foreach ( $options as $v => $t ) {
			if ( is_bool( $t ) ) {
				$r = $this->getRequest()->getBool( $v, $options[$v] );
			} elseif ( is_int( $t ) ) {
				$r = $this->getRequest()->getInt( $v, $options[$v] );
			} elseif ( is_string( $t ) ) {
				$r = $this->getRequest()->getText( $v, $options[$v] );
			}
