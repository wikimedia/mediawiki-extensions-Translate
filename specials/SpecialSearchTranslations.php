...
class SpecialSearchTranslations extends SpecialPage {
		...
		if ( $filter === 'untranslated' ) {
			...
			try {
				do {
					...
					if ( $offsets['start'] > $offsets['total'] ) {
						$offset = $offsets['start'] - $offsets['total'];
					} else {
						$offset = 0;
					}
	 ...
	 * Filter the message collection to get untranslated messages.
...
HTML
	...
	protected function getSearchInput( $query ) {
		...
		$filter = is_null( $filter ) ? '' : Html::hidden( 'filter', $filter );
