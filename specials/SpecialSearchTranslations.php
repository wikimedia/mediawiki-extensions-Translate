	...
	public function execute( $par ) {
		...
		try {
			if ( $filter !== '' ) {
				...
				$actuallimit = $limit = $opts->getValue( 'limit' );
				$size = 1000;

				$opts->setValue( 'limit', $size );
				$opts->setValue( 'language', $opts->getValue( 'sourcelanguage' ) );
				do {
				...
				$opts->setValue( 'offset', $actualoffset );
				$opts->setValue( 'limit', $actuallimit );
			} else {
				$resultset = $server->search( $queryString, $opts, $this->hl );
...
HTML
	...
	protected function messageSelector() {
		...
		$text = $this->msg( 'tux-sst-from' )->text() .
			$sourcelanguage .
			$this->msg( 'tux-sst-of' )->text();
