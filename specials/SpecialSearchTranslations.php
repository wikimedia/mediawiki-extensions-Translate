...
class SpecialSearchTranslations extends TranslateSpecialPage {
	...
	protected function viewPrevNext( Title $title, $offset, $limit,
		...
		# Make 'previous' link
		$prev = $this->msg( 'tux-sst-prev' )->inLanguage( $this )->title( $title )->numParams( $limit )->text();
		...
		# Make 'next' link
		$next = $this->msg( 'tux-sst-next' )->inLanguage( $this )->title( $title )->numParams( $limit )->text();
