...
class SpecialManageGroups extends SpecialPage {
	...
	protected function handleRenameSubmit( MessageGroup $group, MessageSourceChange $sourceChanges,
		...
		foreach ( $renames as $key => $params ) {
			...
			if ( !$req->getCheck( $id ) ) {
				// Needs additional checks for renamed messages as the keys in the wiki
				// are not submitted along with the request, only the incoming modified
				// keys are submitted, so check for those as well
	...
	protected function handleModificationsSubmit( MessageGroup $group,
		...
		// Ignore renames
		unset( $subchanges[ MessageSourceChange::M_RENAME ] );
	...
	protected function isTitlePresent( Title $title, $type ) {
		if ( $title && ( $type === MessageSourceChange::M_DELETION ||
			$type === MessageSourceChange::M_CHANGE ) && !$title->exists() ) {
