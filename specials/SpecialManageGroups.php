...
class SpecialManageGroups extends SpecialPage {
	...
	protected function formatChange( $group, $code, $type, $params ) {
		...
		if ( $title->exists() ) {
			...
			$actions .= ' ' . Xml::checkLabel( $this->msg( 'translate-manage-action-ignore' )->plain(), "i/$id", "i/$id" );

			if ( $group->getSourceLanguage() === $code ) {
				$actions .= ' ' . Xml::checkLabel( $this->msg( 'translate-manage-action-fuzzy' )->plain(), "f/$id", "f/$id" );
			}
		}
