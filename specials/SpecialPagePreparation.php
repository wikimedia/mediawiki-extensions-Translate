...
class SpecialPagePreparation extends SpecialPage {
	...
	function execute( $par ) {
		...
		$out = <<<HTML
<div class="grid">
	<form class="mw-tpp-sp-form row" id="mw-tpp-sp-primary-form" name="mw-tpp-sp-input-form">
		<input name="title" id="title" class="mw-searchInput"
			placeholder= "{$this->msg( 'pp-pagename-placeholder' )->text()}" />
		<input id="action-prepare" class="mw-ui-button mw-ui-primary" type="button"
			value="{$this->msg( 'pp-prepare-button-label' )->text()}" />
		<input id="action-save" class="mw-ui-button mw-ui-constructive hide" type="button"
